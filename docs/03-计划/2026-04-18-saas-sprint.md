...
| # | 缺口 | 影响评分 | 优先级 |
|---|---|---|---|
| G1 | **晴天 MCP 实际接入后端** | 差异化核心 + 传播分析报告数据源 | **P0** | ✅ 已完成 |
| G2 | **版本 diff 前端可视化** | 改写 25% + 版本 15% | **P0** | ✅ 已完成 |
| G3 | **流式打字光标 + Markdown 渐进渲染** | AI 感 + 演示第一印象 | **P0** | ✅ 已完成 |
| G4 | **Demo 数据铺垫** | 赛题硬性交付 | **P0** | ✅ 已完成 |
| G5 | **报告详情页 + 版本树 UI** | 版本 15% | **P1** | ✅ 已完成 |
| G6 | **知识库搜索 UI** | 检索 25% | **P1** | ✅ 已完成 |
| G7 | **段落级改写按钮** | 改写 25% 细腻度 | **P1** | ✅ 已完成 |
| G8 | **AI 生成进度可视化** | AI 感 + 演示气质 | **P1** | ✅ 已完成 |
| G9 | **必交材料闭环** | 赛题硬性要求 | **P0** | ⏳ 部分完成 |

### ✅ SaaS 化产品第二轮补充完成项

| # | 补充项 | 状态 | 说明 |
|---|--------|------|------|
| S1 | MCP 参数格式修正 | ✅ | article-search 使用 articleInfo 嵌套结构；sass-mcp 使用 searchKeywordType+mustKeyWord+realTime |
| S2 | 报告详情页导出功能 | ✅ | Word(后端POI) + PDF(前端html2pdf) 双格式导出 |
| S3 | Docker 安全加固 | ✅ | backend 改用 reportai 用户连 MySQL + MCP appkey 环境变量 |
| S4 | Dashboard 动态统计 | ✅ | todayGenerated 改为查询当日生成数 |
| S5 | 段落级改写按钮 | ✅ | hover 段落浮出改写/扩写/精简 + 后端 rewrite-section SSE 端点 |
| S6 | MCP article-detail 参数修正 | ✅ | 改为 uuid + publishTime 必填参数 |

### ✅ SaaS 化产品第三轮完成项（竞赛100% + 产品化）

| # | 补充项 | 状态 | 说明 |
|---|--------|------|------|
| S7 | FULLTEXT ngram 中文支持 | ✅ | 3张表FULLTEXT索引添加 `WITH PARSER ngram` |
| S8 | Demo数据补全 | ✅ | 10篇报告(5类x2), 17个版本记录(含4种改写模式demo) |
| S9 | 修订痕迹视图 | ✅ | 报告详情页新增"修订痕迹"模式，支持接受/拒绝操作 |
| S10 | 大纲导航 | ✅ | 报告详情页左侧大纲导航，点击跳转到对应章节 |
| S11 | 版本回滚 | ✅ | 版本侧边栏回滚按钮 + 前端API + 后端restore接口 |
| S12 | 报告库卡片视图 | ✅ | 卡片/表格双模式切换，卡片hover动效 |
| S13 | 模板中心风格预览 | ✅ | 风格描述+结构标签展示 |
| S14 | 引用溯源增强 | ✅ | 进度条式相关度可视化 |
| S15 | 全局主题统一 | ✅ | indigo(#6366f1)体系，全局SCSS变量+组件配色统一 |
| S16 | 操作日志AOP切面 | ✅ | OperationLogAspect自动记录所有Controller操作 |
| S17 | 安全认证修复 | ✅ | 无token直接返回401，不再设置匿名认证 |
| S18 | 批量上传 | ✅ | 知识库el-upload添加multiple属性 |
| S19 | 知识库页面美化 | ✅ | 配色统一+圆角+hover动效 |

### ✅ 第四轮完成项（对标晴天大模型报告需求）

| # | 补充项 | 晴天优先级 | 状态 | 说明 |
|---|--------|-----------|------|------|
| S20 | AI对话浮窗 | P0 | ✅ | 多轮对话+建议标签+插入到报告+后端/api/v1/reports/ai-chat |
| S21 | 空白处唤起AI | P0 | ✅ | Ctrl+Enter快捷键+底部AI辅助栏+提示文案 |
| S22 | 中英翻译 | P0 | ✅ | 自动检测语言方向+调用AI翻译 |
| S23 | 一键排版 | P1 | ✅ | Markdown格式规范化(多余空行/标题格式) |
| S24 | 图表下方标题 | P2 | ✅ | "图N：XXX分布"格式+主题描述 |
| S25 | 图表配色统一 | - | ✅ | indigo色系8色方案+渐变柱状图 |
