...
| 2026-04-20 22:15 | Bolt + Pixel | Claude Code (Opus 4.7) + javap + curl + ES 9200 | 现场答辩前定位"右侧引用溯源 0 条"：用 `javap -p` 反编译运行中的 `report-hub-api-1.0.0.jar` 内嵌 `RagSearchServiceImpl`，发现 IDE 跑的是 `11d5a69` 之前的旧字节码（仍含 `isCjk(char)` + `toBooleanModeExpression` `+单字 +单字` 路径，与现行 ngram bigram 索引零交集 → MySQL 实测 hit_count=0）；同时 `21:50` 的 fat jar 时戳是误导，子模块 class 是更早的；ES 索引 `knowledge_chunks` 1936 条/kb=3 427 条，curl ES 直查 391 命中证明数据无问题。修复：mvn clean package + 清 `redis rag:*` cache + 重启后端，curl `/api/v1/knowledge/search?kbId=3` 返 5 命中验证走通 ES 通路 | 本次修复 |
| 2026-04-20 22:25 | Bolt | Claude Code (Opus 4.7) | 修后端 `Doubao stream 失败：未配置 report-ai.llm.doubao.endpoint`：第一次 `nohup java -jar` 未加载 `.env`，DOUBAO_ENDPOINT 为空。重启用 `set -a && . ./.env && set +a` 把 .env 注入子进程环境后再起 JVM；smoke `POST /api/v1/reports + GET /generate` 一遍正常返回 token 流 | — |
| 2026-04-20 22:30 | Bolt | Claude Code (Opus 4.7) | 顺手修引用溯源相关度条全满格：`searchFromEs` 之前对所有 hit `setScore(1.0)` 丢了 ES `_score`。`EsChunkDocument` 加 `@JsonIgnore transient score`、`EsChunkService.collectHits` 把 `Hit.score()` 写进 doc、`RagSearchServiceImpl` 按批次内 max 线性归一化（top=1.0 其他按比例缩，多 KB enhanced 各自归一避免量级抖动）；验证：同一 query top5 score = 1.0000 / 0.9430 / 0.9316 / 0.9161 / 0.8805，UI 进度条恢复分布感 | `6fb421a` |
| 2026-04-20 22:40 | Leo + Bolt | Claude Code (Opus 4.7) + bash + python + SSE 客户端 | 交付脚本 `scripts/demo-deliverables.sh` 一键生成评委要的演示数据：3 主题（政策影响 / 传播分析 / 行业分析）× 1 首次 + 4 改写（DATA_UPDATE/ANGLE_SHIFT/EXPAND/STYLE_SHIFT）+ 首份额外 1 续写。SSE 消费用 Python 解析 event 帧 + 局部 disable pipefail 吃 curl exit 18（SSE 优雅断流的常见返回码）。同步补 `report-ai/README.md`（启动 + 5 模块对照赛题 + 交付资产表 + 故障速查）| 本 commit |
